...
import org.mule.modules.wikipedia.processors.QueryPropGlobalUsageMessageProcessor;
...
public class QueryPropGlobalUsageDefinitionParser
    extends AbstractDevkitBasedDefinitionParser
{

    private static Logger logger = LoggerFactory.getLogger(QueryPropGlobalUsageDefinitionParser.class);

    private BeanDefinitionBuilder getBeanDefinitionBuilder(ParserContext parserContext) {
        try {
            return BeanDefinitionBuilder.rootBeanDefinition(QueryPropGlobalUsageMessageProcessor.class.getName());
            ...
            logger.error(("Cannot launch the mule app, the @Processor [query-prop-global-usage] within the connector [wiki-pedia] is not supported in mule "+ muleVersion));
            throw new BeanDefinitionParsingException(new Problem(("Cannot launch the mule app, the @Processor [query-prop-global-usage] within the connector [wiki-pedia] is not supported in mule "+ muleVersion), new Location(parserContext.getReaderContext().getResource()), null, noClassDefFoundError));
        }
    }

    public BeanDefinition parse(Element element, ParserContext parserContext) {
        BeanDefinitionBuilder builder = getBeanDefinitionBuilder(parserContext);
        builder.addConstructorArgValue("queryPropGlobalUsage");
        builder.setScope(BeanDefinition.SCOPE_PROTOTYPE);
        if (!hasAttribute(element, "config-ref")) {
            throw new BeanDefinitionParsingException(new Problem("It seems that the config-ref for @Processor [query-prop-global-usage] within the connector [wiki-pedia] is null or missing. Please, fill the value with the correct global element.", new Location(parserContext.getReaderContext().getResource()), null));
        }
        ...
        parseProperty(builder, element, "guprop", "guprop");
        parseProperty(builder, element, "gulimit", "gulimit");
        parseProperty(builder, element, "gucontinue", "gucontinue");
        parseProperty(builder, element, "gufilterlocal", "gufilterlocal");
